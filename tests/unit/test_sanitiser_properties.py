...
class TestNormalizeUnicodeProperties:
    ...
    def test_normalize_idempotent(self, text):
        ...
        Note: This is only true for ASCII-preserving inputs. Non-ASCII text
        may be normalized differently on subsequent passes (e.g., Greek alpha
        'α' → 'a' on second pass is CORRECT - we're catching homoglyphs).
        This test verifies that ASCII text is stable.
        """
        if text.isascii():
...
class TestCompactInjectionProperties:
    ...
    def test_plain_text_no_compact_injection(self, text):
        ...
        _ = _detect_compact_injection(text)
...
class TestSanitiserRegression:
    ...
    def test_known_injections_detected(self, text):
        ...
        _ = any(p.search(result.normalised_text) for p in INJECTION_PATTERNS)
